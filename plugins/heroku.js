...
bot(
	...
	async (message, match) => {
		if (!match)
			return await message.sendMessage(`_Example: .setvar mod:919876543210_`)
...
bot(
	...
	async (message, match) => {
		if (!match) return await message.sendMessage(`_Example: delvar mod_`)
		heroku
...
bot(
	...
	async (message, match) => {
		if (!match) return await message.sendMessage(`_Example: getvar mod_`)
		const key = match.trim().toUpperCase()
		heroku
			.get(baseURI + '/config-vars')
			.then(async (vars) => {
